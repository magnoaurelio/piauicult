...
class DiscoList extends TPage
{
    ...
    public function __construct()
    {
       ...
       // $disnome = new TEntry('disnome');
        $disnome = new TDBCombo('discodigo','conexao','Disco','discodigo','{disnome} - {discodigo}','disnome');
     ...
     //   $this->form->addQuickField('Código', $discodigo,  200 );
        ...
        $column_artcodigo = new TDataGridColumn('Artista->artfoto', 'Artista', 'right');
  ...
  // criar na grade um ordenação
       ...
       // $this->datagrid->addColumn($column_disfundo);
        ...
        // PRODUTORES
        $action_edit = new TDataGridAction(array('DiscoBanda', 'onLoad'));
        $action_edit->setUseButton(TRUE);
        $action_edit->setButtonClass('btn btn-default');
