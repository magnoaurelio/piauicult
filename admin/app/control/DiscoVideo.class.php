 ...
 */
class DiscoVideo extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        $this->form->setFormTitle('Disco (Video): <b><span id="artnome"><span></b>');
        
        $inscodigo = new TDBCombo('vidcodigo','conexao','Videos','vidcodigo','{viddescricao{-{vidcodigo}','viddescricao');
        $this->form->addQuickField('Video', $inscodigo,  300 );
        ...
        $this->form->addQuickAction('Adicionar Video', new TAction(array($this, 'onSearch')), 'fa:save green');
        ...
        $column_artcodigo = new TDataGridColumn('vidcodigo', 'Código', 'right', 60);
        $column_artnome = new TDataGridColumn('viddescricao', 'Nome', 'left');


 // criar na grade um ordenação nome
        $order_artnome = new TAction(array($this, 'onReload'));
        $order_artnome->setParameter('order', 'viddescricao');
        ...
        $container->add(TPanelGroup::pack('Disco (Video) : <b><span id="artnome"><span></b>', $this->form));
    ...
    public function onSearch($param)
    {
       TTransaction::open('conexao');
        ...
        if(!empty($disco->vidcodigo)){
            $ids = explode(';',$disco->vidcodigo);
        }else{
            $ids = [];        
        }
        if(!in_array($param['vidcodigo'],$ids) and !empty($param['vidcodigo'])){
         $ids[] = $param['vidcodigo'];
        }
       $disco->vidcodigo  = implode(';',$ids);
       $disco->store();
       TTransaction::close();
       $this->onReload();
    }
