 ...
 */
class DiscoBanda extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        $this->form->setFormTitle('Disco (Produtor) -  Disco: <b><span id="artnome"><span></b>');
 ...
 // criar na grade um ordenação nome
        ...
        $container->add(TPanelGroup::pack('Disco (Grupo) -  Disco: <b><span id="artnome"><span></b>', $this->form));
    ...
    public function onSearch($param)
    {
       TTransaction::open('conexao');
        $collections = BandaDisco::where("discodigo", "=", TSession::getValue('discodigo'))
            ->where("bancodigo","=", $param["bancodigo"])->load();
        if (!$collections){
            $bd = new BandaDisco();
            $bd->discodigo = TSession::getValue('discodigo');
            $bd->bancodigo = $param["bancodigo"];
            $bd->store();
        }else{
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            $repository = new \Adianti\Database\TRepository('BandaDisco');
            $limit = 10;
            // creates a criteria
            $criteria = new \Adianti\Database\TCriteria;
            $criteria->add(new \Adianti\Database\TFilter("discodigo","=",TSession::getValue('discodigo')));
            ...
            foreach ($repository->load($criteria) as $bandaDisco)
            {
                $object = new Banda($bandaDisco->bancodigo);
    ...
    {
        TTransaction::open('conexao');
        BandaDisco::where("discodigo", "=", TSession::getValue('discodigo'))
                    ->where("bancodigo","=", $param["bancodigo"])
                    ->delete();
        TTransaction::close();
        $this->onReload();
    }
