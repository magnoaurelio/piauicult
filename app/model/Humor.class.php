...
class Humor extends Read {
    ...
    public function __construct($id = null, $criterio =  null) {
        if ($id) {
            parent::ExeRead($this->Table, "WHERE humcodigo = :id", "id={$id}");
        } else {
    ...
    function getCategoria(){
        $categoria = new HumorCategoria($this->humcategoria);
        return $categoria;
    }
...
//    }
    ...
    function getPagina(){
        parent::parseQuery("SELECT lv.* from humor as h 
        join festival as f join livro_pagina as lv 
        join livro as l 
        on f.fescodigo = h.fescodigo and l.fescodigo = f.fescodigo and lv.livcodigo = l.livcodigo and lv.numero = h.humpagina
        WHERE f.fescodigo = ".$this->fescodigo." AND lv.numero = ".$this->humpagina);
